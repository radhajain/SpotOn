...
firebase_common_1.firebase.login = function (arg) {
    return new Promise(function (resolve, reject) {
        try {
            var onCompletionWithUser_1 = function (user, error) {
                if (error) {
                    // also disconnect from Google otherwise ppl can't connect with a different account
                    if (typeof (GIDSignIn) !== "undefined") {
                        GIDSignIn.sharedInstance().disconnect();
            ...
            var onCompletionWithAuthResult_1 = function (authResult, error) {
                onCompletionWithUser_1(authResult && authResult.user, error);
            };
                ...
                if (fAuth_2.currentUser) {
                    ...
                    var onCompletionLink = function (user, error) {
                        if (error) {
                            ...
                            fAuth_2.signInWithCredentialCompletion(fIRAuthCredential_1, onCompletionWithUser_1);
                        }
                        else {
                            onCompletionWithUser_1(user);
                        }
                    };
                    ...
                    firebase_common_1.firebase.requestPhoneAuthVerificationCode(function (userResponse) {
                        ...
                        if (fAuth_2.currentUser) {
                            var onCompletionLink = function (user, error) {
                                if (error) {
                                    // ignore, as this one was probably already linked, so just return the user
                                    fAuth_2.signInWithCredentialCompletion(fIRAuthCredential, onCompletionWithUser_1);
                                }
                                else {
                                    onCompletionWithUser_1(user);
                                }
                        ...
                        if (fAuth_2.currentUser) {
                            ...
                            var onCompletionLink = function (user, error) {
                                if (error) {
                                    // ignore, as this one was probably already linked, so just return the user
                                    log("--- linking error: " + error.localizedDescription);
                                    fAuth_2.signInWithCredentialCompletion(fIRAuthCredential_2, onCompletionWithUser_1);
                                }
                                else {
                                    onCompletionWithUser_1(user);
                                }
                    ...
                    if (error === null) {
                        ...
                        if (fAuth_2.currentUser) {
                            ...
                            var onCompletionLink = function (user, error) {
                                if (error) {
                                    // ignore, as this one was probably already linked, so just return the user
                                    fAuth_2.signInWithCredentialCompletion(fIRAuthCredential_3, onCompletionWithUser_1);
                                }
                                else {
                                    onCompletionWithUser_1(user);
                                }
